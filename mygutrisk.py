...
# 填写预测变量
# 社会人口学
factor1 = st.radio('a1性别', ['男性', '女性'])
factor2 = st.slider('trueage请填写您的年龄', 45, 120)
factor3 = st.radio('a2请填写您的民族', ['汉族', '其他'])
factor4 = st.radio('a53a4请选择您的学历',
                   ['小学及以下', '小学学历', '初中学历', '高中学历', '大学及以上', '从未上过学', '不清楚'])
factor5 = st.radio('f41请选择您的婚姻状态', ['已婚', '离异', '丧偶', '未婚', '不清楚'])
factor6 = st.radio('b11您的生活水平如何？', ['很好', '好', '一般', '不好', '很差', '不清楚'])
factor7 = st.radio('b12您觉得您的身体状况如何？', ['很好', '好', '一般', '不好', '很差', '不清楚'])
factor8 = st.radio('b23您觉得您精力充沛吗？', ['总是', '经常', '有时', '偶尔', '从不', '不清楚'])
# 行为学
factor9 = st.radio('d1您平常主食以什么为主？', ['大米', '全麦谷物', '面粉', '米面各一半', '其他', '不清楚'])
factor10 = st.radio('d31您吃新鲜水果的频率如何？', ['几乎每天吃', '经常吃', '有时吃', '很少或从不吃', '不清楚'])
factor11 = st.radio('d32您吃新鲜蔬菜的频率如何？', ['几乎每天吃', '经常吃', '有时吃', '很少或从不吃', '不清楚'])
factor12 = st.radio('d34您的口味主要是什么？', ['清淡', '偏咸', '偏甜、辣、生冷', '没有以上习惯'])
...
factor24 = st.radio('您喝茶的频率如何？',
                    ['几乎每天吃', '不是每天，但每周至少一次', '不是每周，但每月至少一次', '不是每月，但有时吃',
                     '很少或从不吃', '不清楚'])
factor25 = st.radio('d6a您的饮用水主要是什么？', ['井水', '河水或湖水', '泉水', '塘水', '自来水（含纯净水）', '不清楚'])
factor26 = st.radio('d71您现在是否吸烟？', ['是', '否', '不清楚'])
factor27 = st.radio('d72您过去是否吸烟？', ['是', '否', '不清楚'])
factor28 = st.radio('d81您现在是否经常喝酒？', ['是', '否', '不清楚'])
factor29 = st.radio('d82您过去是否经常喝酒？', ['是', '否', '不清楚'])
factor30 = st.radio('d11a您做家务的频率？', ['每天', '至少一周一次', '至少一月一次', '偶尔', '从不', '不清楚'])
# 疾病史
factor31 = st.radio('n您是否患有关节炎？', ['是', '否', '不清楚'])
factor32 = st.radio('j您是否患有前列腺疾病？', ['是', '否', '不清楚'])
factor33 = st.radio('q您是否患有胆囊炎或胆石症？', ['是', '否', '不清楚'])
factor34 = st.radio('r您是否患有血脂异常？', ['是', '否', '不清楚'])
factor35 = st.radio('t您是否患有慢性肾炎？', ['是', '否', '不清楚'])
factor36 = st.radio('v您是否患有子宫肌瘤？', ['是', '否', '不清楚'])
factor37 = st.radio('g您是否患有白内障？', ['是', '否', '不清楚'])
factor38 = st.radio('i您是否患有癌症？', ['是', '否', '不清楚'])
factor39 = st.radio('c您是否患有心脏病？', ['是', '否', '不清楚'])
factor40 = st.radio('d您是否患有中风及脑血管疾病？', ['是', '否', '不清楚'])
factor41 = st.radio('s您是否患有风湿或类风湿？', ['是', '否', '不清楚'])
factor42 = st.radio('u您是否患有乳腺增生？', ['是', '否', '不清楚'])
factor43 = st.radio('f66您童年时期是否经常挨饿？', ['是', '否', '不清楚'])
# 体格检查
factor44 = st.slider('g101请填写您的体重（kg）', -1, 150)
factor45 = st.slider('g1021请填写您的身高（cm）', -1, 200)
factor46 = st.slider('g102c请填写您的腰围（cm）', -1, 130)
...
def make_predict(input_df):
    # Load the trained model for predictions
    model = joblib.load("/Users/galinsoga/22023482_Xiao/New Gastric Ulcers/Final Results/2018/Model Parameters/sklearn_RF_best_model.sav")  # 使用joblib导入保存好的模型
